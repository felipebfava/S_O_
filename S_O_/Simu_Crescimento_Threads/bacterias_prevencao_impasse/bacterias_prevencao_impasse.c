...
void *simular_crescimento(void *arg) {
    ...
    for (int t = 0; t < colonia->tempo_total; t++) {
        ...
        //todas as colônias tentam obter os recursos na mesma ordem: recurso A (Nutrientes) e depois o recurso B (Espaço)
        ...
        //calcular a população usando a fórmula de crescimento exponencial, com população como ponto flutuante de 2 casas decimais
	...
	while(1) {
		...
		for(int i = 0; i < 100; i++) {
			if(status[i] == 1 && prev_status[i] == 1) {
				wait_count[i]++;
				if(wait_count[i] > 3) {
					// Se a thread ainda estiver esperando depois de 3 verificações, pode ter acontecido um deadlock
